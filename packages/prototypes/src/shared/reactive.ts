...
import { onBeforeUnmount, shallowRef, computed as vueComputed, watch as vueWatch, watchEffect as VueWatchEffect } from 'vue-demi'
import { autorun, reaction } from '@formily/reactive'

export * from 'vue-demi'

export function computed<T>(calc: ComputedGetter<T>): ComputedRef<T> {
...
export function watch(deps: Parameters<Watch>[0], callback: Parameters<Watch>[1], options?: Parameters<Watch>[2]): ReturnType<Watch> {
