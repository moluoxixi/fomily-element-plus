...
import { defineComponent, IconWidget, TextWidget, usePrefix, useTheme } from '@moluoxixi/element-prototypes'
...
const MonacoInputInner = defineComponent({
  ...
  setup(props, { attrs, emit }) {
    ...
    const renderHelper = () => {
      ...
      const getHref = () => {
        if (typeof props.helpLink === 'string')
          return props.helpLink
        if (isFileLanguage()) {
    ...
    return () => {
      ...
      realLanguage.value = /javascript|typescript/i.test(computedLanguage.value) ? 'typescript' : computedLanguage.value
